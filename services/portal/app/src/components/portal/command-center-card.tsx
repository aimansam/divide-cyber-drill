 ...
 *
 *   * "How many drills are live right now?"
 *   * "What can I do from here?"
 *   * "What happened recently?"
 *   * "Is the platform healthy?"
 *
 * Replaces the previous DashboardCard with a more action-oriented
 * layout. In a real cyber range this view is the first thing a team
 * lead opens in the morning.
 *
...
export function CommandCenterCard({
  ...
  if (!loading && runs.length === 0 && meRole) {
    return (
      <div data-testid="command-center-card" className="space-y-4">
  ...
  return (
    <div data-testid="command-center-card" className="space-y-6">
      ...
      {/* Status Bar */}
      <section aria-label="Status bar" className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <div className="rounded-lg border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Activity className="h-4 w-4" />
            <span>Live drills</span>
          </div>
          <div className="mt-1 text-2xl font-bold">{stats.live}</div>
          <div className="text-xs text-muted-foreground">
            {stats.live === 0 ? "no active drills" : "running right now"}
          </div>
        </div>
        <div className="rounded-lg border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <div className="mt-1 text-2xl font-bold">{stats.todayCount}</div>
          <div className="text-xs text-muted-foreground">runs started today</div>
        </div>
        <div className="rounded-lg border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {visibleActions.map((action) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-start gap-1 rounded-lg border border-border bg-card p-3 text-left transition-colors hover:bg-accent"
              >
