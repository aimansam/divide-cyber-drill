...
This is the *target* permission set once `require_role(...)` is wired.
None of these gates exist today; they're enumerated here so the next
time we add an endpoint we know which roles it belongs to.
...
| No "own runs only" filter | everyone | privacy + multi-tenant prep | L3 3.12 (org model) |
